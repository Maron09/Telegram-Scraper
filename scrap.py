...
async def get_group_members(group_link, filter_choice, phone):
    async with TelegramClient(f'sessions/{phone}', api_id, api_hash) as client:
        ...
        print(f"{messages['info']}Select output format:\n{colors['red']}[1]{colors['light_green_ex']} JSON\n{colors['red']}[2]{colors['light_green_ex']} Excel")
        fmt = input(f"\n{messages['info']}Enter your choice (1 or 2): {colors['cyan']}")
        if fmt == '1':
            write_to_json(filtered_members, group.title, group.id)
        elif fmt == '2':
            write_to_excel(filtered_members, group.title, group.id)
        else:
            print(f"{messages['error']}Invalid choice. Please select either 1 or 2.")
...
def write_to_excel(members, group_title, group_id):
    data = [{
        'Name': member['name'],
        'Username': member['username'],
        'User ID': member['user_id'],
        'Access Hash': member['access_hash'],
        'Group': group_title,
        'Group ID': group_id,
        'Status': type(member['status']).__name__,
    ...
    df.to_excel(f"{group_title}.xlsx", index=False)

    print (f"{messages['info']}Saved! File Name: {colors['yellow']}{group_title}.xlsx")
